...
def GetDataset(name, base_path):
    ...
    if name=="star":
        ...
        data_names = data_names[data_names != 'gender']
        data_names = np.append(data_names, 'gender')
    ...
    if name=="bike":
        ...
        df = df[[col for col in df.columns if col != 'workingday'] + ['workingday']]
    ...
    if name=="community":
        ...
        # Assuming 'data' is your DataFrame with the column 'OtherPerCap' that contains missing values
        ...
        race_binary = 1*(data["racepctblack"] <= np.median(data["racepctblack"]))
        data["race_binary"] = race_binary
        X = data.iloc[:, np.r_[0:100, data.columns.get_loc("race_binary")]].values
