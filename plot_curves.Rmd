...
data[data$method == "Conditional (kernel)", "method"] <- "Conditional (Gibbs)"
data[data$method == "marginal", "method"] <- "Marginal"
...
preds_oracle <- data.table(X = as.vector(data_test$X),
                           f = data_test$mu, 
                           lower = qnorm(0.05, data_test$mu, data_test$sigma2),
                           upper = qnorm(0.95, data_test$mu, data_test$sigma2),
                           method = "Oracle")
...
data$method <- factor(data$method, levels = c("Isotonic (ours)", "Conditional (Gibbs)", "Marginal"))
...
plt <- ggplot(data, aes(x = X, color = method, linetype = method)) +
  ...
  theme(legend.position="bottom") + theme_bw() + labs(color = "", y = "Y") + facet_wrap(~ n, labeller = as_labeller(appender, 
                            default = label_parsed) ) +  labs(color  = "", linetype = "", shape = "")   + theme(legend.key.size = unit(1, "cm"), legend.position = "bottom", legend.direction = "horizontal",
